...
pub fn run() {
  let app = tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      let binaries_manager = BinariesManager::new(handle);
      if let Err(e) = binaries_manager.extract_bundled_if_needed() {
        tracing::warn!("Failed to extract bundled yt-dlp: {e}");
      }
      handle.manage(binaries_manager);
