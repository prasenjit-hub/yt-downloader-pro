...
impl BinariesManager {
  ...
  const BUNDLED_BINARY_RESOURCE: &str = "binaries/yt-dlp.exe";

  /// Checks if the app was built with a bundled yt-dlp and copies it
  /// into the binary directory so the normal download is skipped.
  pub fn extract_bundled_if_needed(&self) -> Result<(), AnyError> {
    let canonical = self.canonical_path("yt-dlp")?;
    ...
    // Look for the binary inside the app's resource directory (bundled at build
    // time via tauri.conf.json  →  bundle.resources).
    let resource_dir = self.app.path().resource_dir()?;
    let bundled: PathBuf = resource_dir.join(Self::BUNDLED_BINARY_RESOURCE);

    if !bundled.exists() {
      tracing::info!("No bundled yt-dlp binary at {:?}", bundled);
      return Ok(());
    }
    ...
    tracing::info!(
      "Copying bundled yt-dlp from {:?} to {:?}",
      bundled,
      canonical,
    );
    std::fs::copy(&bundled, &canonical)?;
